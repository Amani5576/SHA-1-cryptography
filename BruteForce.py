...
#uses recursion
def brute_force(digest, p_len, rep, count, word, char_s, alph_len, **kwargs):
    ...
    for i in range(alph_len):
        ...
        #recurse
        brute_force(digest, p_len, rep, count, word, 
                    char_s, alph_len, **kwargs)
        
    return

def BF(d, p_len, rep, char_s):
    
    #Digest of the very word being investigated
    digest = d.upper() 
    
    #p_len # 28 # Password length
    # rep #4 # Number of times letters are allowed to repeat
    count = 0
    ...
    brute_force(digest, p_len, rep, count, word, char_s, alph_len, n = n)
